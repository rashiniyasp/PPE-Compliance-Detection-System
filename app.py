                ...
                while cap.isOpened():
                    ...
                    for res in compliance_results:
                        results_list.append({
                            ...
                            'Detected PPE': ", ".join(res['detected_ppe']) if res['detected_ppe'] else "—",
                            'Confidence': res['conf_str'],
                            'Missing PPE': ", ".join(res['missing_ppe']) if res['missing_ppe'] else "—",
                            'Alert': '⚠️ Yes' if res['alert'] else 'No'
                        })
...
with col_table:
    ...
    if not st.session_state.results_df.empty:
        ...
        with dl_col1:
            ...
            st.download_button(
                label="📥 Download Compliance Logs (CSV)",
                data=csv,
